        ...
        <div className='px-2 pb-10'>
            <div className='h-[70px] sticky top-0 z-50 bg-primary flex items-center justify-between'>
                <div className='rounded-full p-[3px] h-fit bg-[#dde3e3] flex w-fit text-sm'>
                    <button
                        onClick={() => setActBtnPlaylist(true)}
                        className={`px-4 rounded-full py-1 ${actBtnPlaylist && 'text-active shadow-md bg-[#e8eaea]'}`}
                    >
                        Danh sách phát
                    </button>
                    <button
                        onClick={() => setActBtnPlaylist(false)}
                        className={`px-4 rounded-full py-1 ${!actBtnPlaylist && 'text-active shadow-md bg-[#e8eaea]'}`}
                    >
                        Nghe gần đây
                    </button>
                ...
                <button className='rounded-full p-2 bg-[#dde3e3] hover:bg-[#e8eaea] flex items-center justify-center'>
                    <BsFillTrash3Fill
                        onClick={() => dispatch(actions.clearSongRecent())}
                        className='size-5 text-gray-700 hover:text-active'
                    />
                </button>
            </div>
            {actBtnPlaylist && (
            ...
            {!actBtnPlaylist &&
                songRecent?.map(item => <SongRelease key={item.encodeId} song={item} hover={1} size={1} padding={1} />)}
        </div>
    )
}
