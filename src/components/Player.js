                ...
                <div className='flex-auto flex flex-col items-center'>
                    <div className='flex items-center gap-6'>
                        <TbArrowsShuffle
                            ...
                            className={`size-5 cursor-pointer ${isShuffle && 'text-active'}`}
                        />
                        <TbPlayerSkipBackFilled
                            onClick={() => handlePrevious()}
                            className={`size-5 hover:text-active cursor-pointer ${
                                !song && 'cursor-not-allowed text-gray-500'
                            }`}
                        />
                        {!isLoaded && currentSong ? (
                            ...
                            <PiPauseCircle
                                onClick={() => handlePlay()}
                                className='size-10 cursor-pointer hover:text-active'
                            />
                        ) : (
                            <IoPlayCircleOutline
                                onClick={() => handlePlay()}
                        ...
                        <span className='cursor-pointer' onClick={() => handleRepeat()}>
                            {repeat === 'one' ? (
                                ...
                                <TbRepeat
                                    title={repeat === 'all' ? 'Bật phát lại 1 bài' : 'Bật phát lại tất cả'}
                                    className={`size-5 ${repeat === 'all' && 'text-active'}`}
                                />
                            )}
                        </span>
                    </div>
                    <div className='flex gap-3 items-center justify-center select-none w-full'>
                        ...
                        <div
                            ...
                            className='w-[90%] h-1 hover:h-[6px] cursor-pointer bg-gray-400 relative rounded-md group'
                        >
                            <div
                                ref={ref}
                                className={`rounded-md absolute h-full bg-slider-bar top-0 left-0 transition-all ease-linear`}
                            ></div>
                            <div
                                ref={refDot}
                                className='size-3 hidden group-hover:block absolute left-0 top-1/2 -translate-y-1/2 -translate-x-1/2 bg-slider-bar transition-all ease-linear rounded-full'
                            ></div>
                        </div>
                ...
                <div className='w-[30%] flex justify-end gap-3 items-center'>
                    <span className='cursor-pointer' onClick={() => (audioController.muted = !audioController.muted)}>
                        {audioController.muted ? (
                            <IoVolumeMuteOutline className='size-6' />
                        ) : (
                    ...
                    <input
                        ...
                        className='bg-gray-400 appearance-none rounded-md accent-active h-1'
                        type='range'
                        step={0.02}
                        min={0}
                        max={1}
                    ...
                    <span
                        className={`rounded-md p-1 ${
                            isShowRSidebar ? 'bg-slider-bar text-white hover:opacity-90' : 'hover:text-active'
                        }`}
                    >
                        <PiPlaylistFill
                            onClick={() => setIsShowRSidebar(pre => !pre)}
