...
const SongRelease = ({ song, order, percent, isShowDate, text, hover, bg, size, padding }) => {
    ...
    return (
        ...
        >
            {order && (
                <span
                    className={`max-w-[8%] min-w-[8%] text-2xl font-bold fill-transparent outline-2 text-yellow-50
                    ${
                        order === 1
                            ? 'ktn-shadow-top1'
                            : order === 2
            ...
            <div
                ...
                onClick={() => handleClick(song.encodeId)}
                className={`relative cursor-pointer ${padding ? 'min-w-[40px]' : 'min-w-[60px]'}`}
            >
                <img
                    className={`rounded-lg ${size ? 'size-[40px]' : 'size-[60px]'}`}
                    src={song?.thumbnail}
                    alt='thumbnail-song'
                ></img>
                <div className='absolute top-0 left-0 bottom-0 right-0 flex items-center justify-center'>
                    {isPlay && currentSong === song?.encodeId && (
                        <AudioLoading w={padding ? 20 : 30} h={padding ? 20 : 30} border={0} />
                    )}
                    {((currentSong !== song?.encodeId && isHover) || (!isPlay && currentSong === song?.encodeId)) && (
                        <IoPlayCircleOutline className={`${padding ? 'size-8' : 'size-10'} text-white`} />
                    )}
                </div>
            </div>
            <div className='flex flex-auto flex-col justify-center gap-1 overflow-hidden'>
                ...
                <span className={`truncate text-xs ${text ? `${text} font-extralight` : 'text-gray-500'}`}>
                    {song?.artistsNames}
                </span>
                {isShowDate && (
                    <span className='text-xs text-gray-500'>{moment.unix(song?.releaseDate).fromNow()}</span>
                )}
            </div>
