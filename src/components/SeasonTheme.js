...
const SeasonTheme = () => {
    ...
    return(
        <>
            {seasonData?.length > 0 && seasonData.map((item, index) => (
                            ...
                            <Link key={item.link} to={item.link?.split('.')[0]}>
                                <div key={item.encodeId} className="group">
                                    <div className="w-full overflow-hidden hover:opacity-80 rounded-lg">
                                        <img className="w-full cursor-pointer group-hover:scale-110 transition-all duration-500" src={item.thumbnail}></img>
                                    </div>
