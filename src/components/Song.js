...
const Song = ({ song, isAlbum, order }) => {
    ...
    return (
        <div
            className={`${
                currentSong === song?.encodeId && 'bg-sidebar'
            } flex px-3 py-2 rounded-md gap-10 items-center justify-between hover:bg-sidebar border-b-[1px] border-[#0000000d]`}
        >
            <div
                className={`flex items-center gap-2 ${isAlbum ? 'min-w-[50%] max-w-[50%]' : 'min-w-[75%] max-w-[75%]'}`}
            >
                {order ? (
                    <h1
                        className={`mr-2 ${
                ...
                >
                    ...
                    <div className='absolute top-0 left-0 bottom-0 right-0 flex items-center justify-center'>
                        ...
                        {((currentSong !== song?.encodeId && isHover) ||
                ...
                <div className='flex flex-col w-full overflow-hidden'>
                    <h1 className='truncate'>
                        {song?.title.length > 30 ? `${song?.title?.slice(0, 35)}...` : song?.title}
                    </h1>
