        ...
        <div className='w-full overflow-hidden'>
            <Link className='' to={item.link?.split('.')[0]}>
                <div className='w-full overflow-hidden rounded-lg cursor-pointer'>
                    ...
                    >
                        <img className='w-full' src={item.thumbnail} alt='thumbnail'></img>
                        <div className='absolute rounded-lg top-0 right-0 left-0 bottom-0 hover:bg-[#1212126c] transition-all'></div>
                        {isHover && <HoverAlbum />}
                    </div>
                </div>
            ...
            {single && (
                <h1 className=' text-gray-500 hover:text-active cursor-pointer text-sm truncate'>
                        ...
                        (item, index, arr) =>
                            index < 3 && (
                                <div key={index} className='text-xs text-gray-500 inline'>
                                    <Link to={item?.link} className='hover:underline hover:text-active' key={item?.id}>
                                        {item?.name}
                                    </Link>
                                    <span className=''>
                                        {arr.length > 2
