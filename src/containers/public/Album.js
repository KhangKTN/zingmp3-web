...
import { SongList, AudioLoading, Loading } from '../../components'
                ...
                <div className='flex xl:flex-row flex-col w-full gap-10 mt-5'>
                    <div className='xl:w-[400px] w-full flex xl:flex-col flex-row items-center gap-10 xl:gap-1'>
                        <div className='xl:w-[300px] w-[250px] relative cursor-pointer rounded-xl overflow-hidden group'>
                            <div
                                className={`w-full shadow-xl ${
                                    (!isSongOfAlbum || (isSongOfAlbum && !isPlay)) && 'group-hover:scale-110'
                                } transition-all duration-500`}
                            >
                                <img
                                    className={`w-full ${
                                        isSongOfAlbum && isPlay
                                            ? 'rounded-full animate-rotate-center'
                                            : 'rounded-xl animate-rotate-pause'
                                    }`}
                            ...
                            <div
                                className={`${
                                    (!isPlay || !isSongOfAlbum) && 'group-hover:bg-[#1212126c]'
                                } text-white absolute top-0 left-0 bottom-0 right-0 transition-all duration-500 flex items-center justify-center`}
                            >
                                {isPlay && isSongOfAlbum ? (
                                    <AudioLoading w={40} h={40} border={1} />
                                ) : (
                                    <IoPlayCircleOutline className='size-20' />
                                )}
                            </div>
                        </div>
                        <div className='flex flex-auto flex-col xl:text-center'>
                            ...
                            <div className='text-gray-500 flex flex-col xl:items-center gap-1 text-base w-full'>
                                ...
                                <span>
                                    Thời lượng:{' '}
                                    {moment.utc(playlist?.song?.totalDuration * 1000).format('hh [giờ] mm [phút]')}
                                </span>
                                ...
                                <button className='mt-2 px-6 py-2 uppercase bg-slider-bar text-white w-fit rounded-full'>
                                    {isSongOfAlbum && isPlay && (
                                        <span
                                            onClick={() => dispatch(actions.setIsPlay(false))}
                                    ...
                                    {isSongOfAlbum && !isPlay && (
                                        <span
                                            onClick={() => dispatch(actions.setIsPlay(true))}
                                            className='flex items-center gap-3'
                                        >
                                            <FaPlay />
                                            Tiếp tục phát
                                    ...
                                    {!isSongOfAlbum && (
                                        <span onClick={() => handlePlayRandom()} className='flex items-center gap-3'>
                                            <FaPlay />
                                            Phát ngẫu nhiên
                                        </span>
                                    )}
                    ...
                    <div className='flex-auto w-full h-full '>
                        <div className='mb-3 text-gray-600 font-semibold'>
                            Lời tựa:<span className='text-gray-500 font-medium'> {playlist?.sortDescription}</span>
                        </div>
                        <SongList />
                    </div>
                </div>
