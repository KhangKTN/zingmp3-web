...
import { AlbumItem, Artists } from "../../components";
        ...
        <div>
            <div>
                ...
                <div className="grid grid-cols-3 gap-5">
                    ...
                    index < 2 && 
                        <div key={item.id} className="flex gap-3 items-center cursor-pointer rounded-lg bg-[#d7e1e1] p-3 hover:bg-sidebar group">
                            <div className="size-[84px] overflow-hidden rounded-full">
                                <img className="size-[84px] group-hover:scale-105 transition-all" src={item.thumbnail}></img>
                            </div>
                            <Link to={item.link}>
                                <div className="flex flex-col gap-1">
                                    <span className="text-gray-500 font-light text-sm">{item.isOA && 'Nghệ sĩ'}</span>
                                    <span className="font-semibold mt-1 text-[15px] hover:text-active hover:underline">{item.name}</span>
                                    <span className="text-gray-500 font-light text-sm">{convertFollower(item.totalFollow)} quan tâm</span>
                                </div>
                            </Link>
                        </div>
                    ))}
                    {searchData?.songs?.map((item, index) => (
                       index === 0 && <SongRelease key={item.encodeId} bg={'bg-[#d7e1e1]'} song={item} hover={1} />
                    ))}
                </div>
            </div>
            <div className="mt-12">
                <div className="grid grid-cols-4 xl:grid-cols-5 gap-5 w-full mt-3 max-h-[255px] overflow-y-hidden">
                    {searchData?.playlists?.map((item, index) => (
                        index < 5 && <AlbumItem key={item.encodeId} item={item}/>
                    ))}
                </div>
            </div>
            <div>
