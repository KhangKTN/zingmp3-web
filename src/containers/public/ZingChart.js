            ...
            tooltip: {
                ...
                external: context => {
                    ...
                    const result = counters.find(item =>
                        item.data.some(num => num == context.tooltip.body[0]?.lines[0]?.replace('.', ''))
                    )
    ...
    return (
        <>
            {!chartData ? (
                ...
                <div className=''>
                    <h1 className='my-6 font-bold bg-gradient-to-r from-orange-400 via-pink-500 to-indigo-400 inline-block text-transparent text-4xl bg-clip-text'>
                        #zingchart
                    </h1>
                    <div className='flex flex-col gap-y-4 my-3'>
                        <div className='w-full min-h-[300px] relative'>
                            ...
                            >
                                <SongRelease
                                    song={chartData?.RTChart?.items?.find(
                                        item => item.encodeId === selected.songSelected
                                    )}
                                    text={'text-white'}
                                    bg={
                                        selected?.pos === 0
                        ...
                        <div className='w-full flex flex-col justify-center'>
                            {chartData?.RTChart?.items?.map((item, index) => (
                                <div
                                    key={item.encodeId}
                                    className={`bg-[#ffffff21] rounded-md px-3 hover:bg-[#ffffff4a] ${
                                        selected?.songSelected === item.encodeId && 'bg-[#ffffff4a]'
                                    }`}
                                >
                                    {index < (showFull ? 100 : 10) && (
                                        <Song song={item} isAlbum={1} order={index + 1} />
                                    )}
                                </div>
                            ))}
                            <div className='w-full flex justify-center mt-5'>
                                <button
                                    ...
                                    className='border border-active px-4 py-2 rounded-full hover:bg-[#ffffff68]'
                                >
                                    {showFull ? 'Ẩn Top 100' : 'Xem Top 100'}
                                </button>
                            </div>
                        </div>
                    </div>
                        ...
                        <div className='grid grid-cols-1 lg:grid-cols-3 gap-5 my-6'>
                            {weekChart?.map((item, index) => (
                                <div key={index} className='p-2 bg-[#ffffff6a] rounded-xl w-full'>
                                    <h1 className='text-2xl font-bold text-active ml-10 my-4'>
                                        {item.country === 'vn'
                                            ? 'Việt Nam'
                                            : item.country === 'us'
                                            ? 'US - UK'
                                    ...
                                    <Link to={item.link.split('.')[0]} className='w-full flex justify-center'>
                                        <button className='px-4 py-2 mt-3 rounded-full border-active border mx-auto hover:bg-[#ffffffb4]'>
                                            Xem tất cả
                                        </button>
                                    </Link>
                                </div>
