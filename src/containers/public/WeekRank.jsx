...
const WeekRank = () => {
    ...
    return(
        ...
        {!data ? <Loading/> :
        <div>
            <h1 className="text-4xl text-active font-bold capitalize tracking-wide my-10">Bảng xếp hạng tuần</h1>
            <div className="flex text-2xl gap-x-8 mb-4 font-semibold">
            {arr.map(item => (
                <NavLink className={({isActive}) => isActive ? act : noAct} to={data && data[item.key].link.split('.')[0]}>
                    <h1>{item.value}</h1>
                </NavLink>
            ))}
            </div>
